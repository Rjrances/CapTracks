...
<div class="container-fluid mt-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2 flex-wrap align-items-center">
            @if($isGroupLeader)
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createLeaderTaskModal">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="taskCommentsModalLabel{{ $task->id }}">
                        <i class="fas fa-comments me-2"></i>Discussion — {{ $task->task_label }}
                    </h5>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('student.milestones.assign-task', $task->id) }}" method="POST">
                        ...
                        @method('PATCH')
                        <div class="modal-body">
                            <h6>{{ $task->task_label }}</h6>
                            <p class="text-muted">{{ $task->task_body ?? '—' }}</p>
                            @if($task->assigned_to)
...
@endif
@if($isGroupLeader)
    <div class="modal fade" id="createLeaderTaskModal" tabindex="-1" aria-labelledby="createLeaderTaskModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="createLeaderTaskModalLabel">Add task to this milestone</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('student.milestones.store-task', $groupMilestone->id) }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="leader_task_title" class="form-label">Title <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="leader_task_title" name="title" value="{{ old('title') }}" maxlength="255" required>
                            @error('title')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-0">
                            <label for="leader_task_description" class="form-label">Description</label>
                            <textarea class="form-control @error('description') is-invalid @enderror" id="leader_task_description" name="description" rows="4" maxlength="5000">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">New tasks start in <strong>Pending</strong>. You can drag cards between columns or assign them to members like template tasks.</div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
